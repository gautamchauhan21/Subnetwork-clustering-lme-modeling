function run_CommDetect_uniform(input_path, Race, method, nreps_num, NShuff_num)
%RUN_COMMDETECT_UNIFORM Community detection on event-based neural data using a uniform null model
%
% Syntax:
%   run_CommDetect_uniform(input_path, Race, method, nreps_num, NShuff_num)
%
% Inputs:
...
%
% Description:
%   This function performs community detection using a **uniform null model** as described
%   in Bazzi et al. (2015). The model assumes uniform correlations across the matrix, scaled
%   by the gamma resolution parameter. Modularity optimization is repeated over a range of
%   gamma values, and consensus clustering is used to extract robust community assignments.
%
%   Following community detection, neurons are assigned to statistically significant clusters
%   based on participation tested against null distributions from random spike permutations.
%
% Output Files:
%   Results are saved in:
%       [input_path]/Output_CommDetect_Uniform_<method>/
%
...
%   output_result.No_assemblies (%)           : % of event frames with no ensemble recruited
%   output_result.S_assemblies (%)            : % of event frames recruiting one ensemble
%   output_result.M_assemblies (%)            : % of event frames recruiting more than one ensemble
%   output_result.Cells_not_in_assembly (%)   : % of neurons not participating in any ensemble
%   output_result.Cells_in_one_assembly (%)   : % of neurons in exactly one ensemble
%   output_result.Cells_in_many_assembly (%)  : % of neurons in multiple ensembles
%
% Example:
%   run_CommDetect_uniform('session01', RaceMatrix, 'CovM', 500, 5000)
...
output_path = fullfile(input_path, "Output_CommDetect_Uniform_" + method);
if ~exist(output_path, 'dir')
    mkdir(output_path)
end


% Community detection across gamma values
% In our case, it is difficult to decide which gamma should, so we run different gamma value (gammavals)
gammavals = linspace(0,1,11);
nreps = nreps_num;
...
d = zeros(length(rho)); % Cumulative agreement matrix
